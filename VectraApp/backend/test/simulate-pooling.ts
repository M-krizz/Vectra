/* eslint-disable @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-floating-promises */
import { DataSource, Repository } from 'typeorm';
import {
  UserEntity,
  UserRole,
  AccountStatus,
} from '../src/modules/Authentication/users/user.entity';
...
import {
  ...
  RideType,
  VehicleType,
} from '../src/modules/ride_requests/ride-request.enums';
...
async function runSimulation() {
  ...
  try {
    ...
    const userRepo: Repository<UserEntity> =
      SimulationDataSource.getRepository(UserEntity);
    const requestRepo: Repository<RideRequestEntity> =
      SimulationDataSource.getRepository(RideRequestEntity);
    const poolRepo: Repository<PoolGroupEntity> =
      SimulationDataSource.getRepository(PoolGroupEntity);
    const tripRepo: Repository<TripEntity> =
      SimulationDataSource.getRepository(TripEntity);
    const tripRiderRepo: Repository<TripRiderEntity> =
      SimulationDataSource.getRepository(TripRiderEntity);
    ...
    const poolingManager = new PoolingManager(requestRepo, poolingService);

    // 1. Create Riders
    console.log('Creating Test Riders...');
    const riderA: UserEntity = await createOrGetUser(
      userRepo,
      'rider_a_sim@test.com',
      'Rider A Sim',
    );
    const riderB: UserEntity = await createOrGetUser(
      userRepo,
      'rider_b_sim@test.com',
      'Rider B Sim',
    );

    // 2. Create Requests
    const pickup: GeoPoint = { type: 'Point', coordinates: [77.6412, 12.9716] };
    const drop: GeoPoint = { type: 'Point', coordinates: [77.635, 12.92] };
    ...
    const rawMatches = await requestRepo
      ...
      .getMany();
    console.log(
      `Manual Spatial Check Found: ${rawMatches.length} candidates (Radius 1000m)`,
    );
    ...
    console.log(
      `Request A Status: ${reloadA.status} | PoolID: ${reloadA.poolGroupId}`,
    );
    console.log(
      `Request B Status: ${reloadB.status} | PoolID: ${reloadB.poolGroupId}`,
    );

    if (
      reloadA.status === RideRequestStatus.POOLED &&
      reloadB.status === RideRequestStatus.POOLED
    ) {
      console.log('✅ SUCCESS: Both requests are POOLED.');

      if (reloadA.poolGroupId && reloadA.poolGroupId === reloadB.poolGroupId) {
        console.log(
          `✅ SUCCESS: Both belong to same Pool Group: ${reloadA.poolGroupId}`,
        );

        const pool = await poolRepo.findOne({
          where: { id: reloadA.poolGroupId },
        });
        console.log(
          `   Pool Status: ${pool?.status} | Riders: ${pool?.currentRidersCount}`,
        );

        const tripRiderEntry = await tripRiderRepo.findOne({
          where: { riderUserId: riderA.id },
        });
        if (tripRiderEntry) {
          const trip = await tripRepo.findOne({
            where: { id: tripRiderEntry.tripId },
          });
          console.log(
            `✅ SUCCESS: Trip Created: ${trip?.id} | Status: ${trip?.status}`,
          );
        } else {
        ...
        console.error(
          '❌ FAILURE: Requests have different or missing Pool Group IDs.',
        );
      }
    } else {
    ...
    if (SimulationDataSource.isInitialized)
      await SimulationDataSource.destroy();
  }
}
