...
export class PoolingV1Update1769875200000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const checkType = (await queryRunner.query(
      `SELECT 1 FROM pg_type WHERE typname = 'vehicle_type_enum'`,
    )) as PgTypeResult[];
    ...
    const checkPoolStatus = (await queryRunner.query(
      `SELECT 1 FROM pg_type WHERE typname = 'pool_groups_status_enum'`,
    )) as PgTypeResult[];
    ...
    // Add vehicle_type
    await queryRunner.query(
      `ALTER TABLE "ride_requests" ADD "vehicle_type" "public"."vehicle_type_enum" NOT NULL DEFAULT 'AUTO'`,
    );

    // Add pool_group_id
    await queryRunner.query(
      `ALTER TABLE "ride_requests" ADD "pool_group_id" uuid`,
    );

    // Update RideRequestStatus Enum to include 'POOLED'
    // This fails inside a transaction block usually.
    await queryRunner.query(
      `ALTER TYPE "public"."ride_requests_status_enum" ADD VALUE IF NOT EXISTS 'POOLED'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "ride_requests" DROP CONSTRAINT "FK_ride_requests_pool_group_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "ride_requests" DROP COLUMN "pool_group_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "ride_requests" DROP COLUMN "vehicle_type"`,
    );
