...
export class PoolingManager {
  ...
  async handlePoolingLoop() {
    ...
    for (const req of activeRequests) {
      ...
      try {
        const candidates = await this.poolingService.findCandidates(
          req,
          radius,
        );
        ...
        if (candidates.length > 0) {
          // Try to form a pool
          const result = await this.poolingService.evaluateGroupings(
            req,
            candidates,
          );
          if (result) {
  ...
  private handleTimeout(req: RideRequestEntity): Promise<void> {
    ...
    this.logger.log(
      `Request ${req.id} timed out searching for pool. Emitting choice event.`,
    );
