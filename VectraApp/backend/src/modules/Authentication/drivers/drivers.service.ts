...
export class DriversService {
    constructor(
        @InjectRepository(DriverProfileEntity) private profileRepo: Repository<DriverProfileEntity>,
    ...
    async updateLicense(userId: string, licenseNumber: string, licenseState?: string) {
    ...
    async getVehicles(userId: string) {
        return this.vehicleRepo.find({ where: { driverUserId: userId, isActive: true } });
    }
    ...
    async suspendDriver(adminUserId: string, driverProfileId: string, reason?: string) {
        const profile = await this.profileRepo.findOne({
            where: { id: driverProfileId },
            relations: ['user'],
        });
