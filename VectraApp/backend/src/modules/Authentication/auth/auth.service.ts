...
export class AuthService {
    ...
    constructor(
        ...
        @InjectRepository(DriverProfileEntity) private profilesRepo: Repository<DriverProfileEntity>,
    ) {
     ...
     */
    async createSessionAndTokens(user: UserEntity, deviceInfo?: string, ip?: string) {
        ...
        const refreshTtlMs = this.parseExpiryToSeconds(this.refreshExpiresIn) * 1000;
    ...
    async rotateRefreshToken(
        ...
        if (!match) {
            ...
            await this.refreshRepo.update({ userId: record.userId }, { revokedAt: new Date() });
        ...
        return await this.createSessionAndTokens(user, deviceInfo ?? record.deviceInfo ?? undefined, ip ?? record.ip ?? undefined);
    }
