...
const testEndpoints = [
  {
    ...
    method: 'GET',
    expectStatus: 401,
    description: 'Auth endpoint (no token)',
  },
  {
    path: '/api/v1/safety/incidents',
    method: 'GET',
    expectStatus: 401,
    description: 'Safety incidents (no auth)',
  },
  {
    path: '/api/v1/profile',
    method: 'GET',
    expectStatus: 401,
    description: 'User profile (no auth)',
  },
];
...
async function testHttpEndpoint(endpoint) {
  return new Promise((resolve) => {
    ...
    req.on('error', (err) => {
      console.log(
        `❌ ${endpoint.description}: Connection failed - ${err.message}`,
      );
      resolve(false);
    });
